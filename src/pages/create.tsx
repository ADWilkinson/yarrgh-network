...
const Create = () => {
	...
	const { data, writeAsync: sendTx, isLoading: txLoading } = useContractWrite(
		{
	...
	return (
		<>
			...
			<form onSubmit={createPost} className="pt-12 space-y-6">
				<Input
					label="Title"
					placeholder="Introducing Yarrgh: A Pirate Network"
					required
					value={title}
					onChange={setTitle}
				/>
				<Input
					label="Link"
					placeholder="https://yarrgh.network/"
					type="url"
					required
					value={link}
					onChange={setLink}
				...
				<Input
					as="textarea"
					label="Description"
					placeholder="Hey Lensters! Just launched Yarrgh, a Pirate Network for.. pirates."
					value={description}
					onChange={setDescription}
					description={
						<span>
							Won&apos;t be shown on Yarrgh, but will make your posts prettier in other Lens sites.{' '}
